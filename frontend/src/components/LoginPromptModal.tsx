...
const LoginPromptModal: React.FC = () => {
  ...
  return (
    ...
    >
      <div 
        className={`bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 max-w-md w-full mx-4 overflow-hidden transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}
      >
        <div className="relative p-6 sm:p-8">
          <button 
            onClick={handleIgnore}
            className="absolute top-4 right-4 p-2 text-lightText/50 hover:text-lightText transition-colors bg-darkBg hover:bg-gray-100 rounded-full"
            aria-label="Close"
          >
          ...
          <div className="flex flex-col items-center text-center mt-2">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 shadow-sm border border-gray-100 ${isCheckout ? 'bg-red-50 text-red-500' : 'bg-darkBg text-primaryAction'}`}>
              {isCheckout ? (
            ...
            <h3 className="text-xl font-serif font-bold text-lightText mb-2">
              {isCheckout ? 'Authentication Required' : 'Save Your Cart'}
            </h3>
            
            <p className="text-sm text-lightText/70 font-light leading-relaxed mb-8 px-2">
              {isCheckout 
            ...
            <div className="flex justify-center w-full">
               <button 
                 onClick={handleLogin}
                 className="btn-primary w-full shadow-md rounded border-transparent"
               >
                 Log in / Sign up
               </button>
            </div>
            ...
            <button 
               onClick={handleIgnore}
               className="mt-4 text-xs font-semibold text-lightText/50 hover:text-primaryAction uppercase tracking-widest transition-colors py-2 px-4 focus:outline-none"
            >
