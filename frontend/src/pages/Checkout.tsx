...
const Checkout = () => {
  ...
  if (cart.length === 0) {
    return (
      <div className="py-24 px-6 text-center fade-in text-lightText">
        ...
        <p className="text-lightText/60 mb-8">You cannot checkout with an empty cart.</p>
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto fade-in text-lightText">
      <h1 className="text-4xl font-bold mb-10 pb-4 border-b border-secondaryAction/20 drop-shadow-[0_0_10px_rgba(253,255,252,0.1)]">Checkout</h1>
      
      <div className="flex flex-col lg:flex-row gap-12">
        <div className="lg:w-2/3">
          <h2 className="text-2xl font-bold mb-6 drop-shadow-sm">1. Delivery Details</h2>
          <form id="checkout-form" onSubmit={handlePlaceOrder} className="premium-card p-8 !rounded-lg border-opacity-50 space-y-6 mb-10">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-lightText/80 mb-2">Full Name *</label>
                <input
                  type="text"
                  name="name"
                  required
          ...
          <h2 className="text-2xl font-bold mb-6 drop-shadow-sm">2. Payment (UPI)</h2>
          <div className="premium-card p-8 !rounded-lg border-opacity-50 space-y-6 relative overflow-hidden">
            {!upiConfig && (
               <div className="absolute inset-0 bg-darkBg/80 flex items-center justify-center z-10 backdrop-blur-sm">
                 <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primaryAction"></div>
               </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-8 items-start">
              ...
              <div className="w-full sm:w-2/3 space-y-5">
                 <div className="bg-secondaryAction/10 text-lightText p-4 rounded-lg text-sm mb-4 border border-secondaryAction/30 leading-relaxed shadow-sm">
                   <strong>Secure Payment Step:</strong> Open your UPI app (GPay, PhonePe, Paytm), scan the QR code to the left or copy the UPI ID, and pay exactly <strong className="text-highlight drop-shadow-sm">{formatPrice(totalAmount)}</strong>. Enter the 12-digit UTR Transaction ID below after paying.
                 </div>

                 <div>
                  <label className="block text-sm font-bold text-lightText mb-2">Transaction ID (UTR) <span className="text-primaryAction">*</span></label>
                  <input
                    type="text"
                    required
                    form="checkout-form"
                    ...
                    maxLength={20}
                  />
                  <p className="text-xs text-lightText/50 mt-1 drop-shadow-sm">Found in your UPI app's payment history (usually 12 digits).</p>
                 </div>

                 <div>
                  <label className="block text-sm font-bold text-lightText mb-2">Payment Screenshot (Optional)</label>
                  <input
                    type="file"
                    accept="image/*"
                    disabled={isPlacingOrder}
                    onChange={(e) => setScreenshotFile(e.target.files?.[0] || null)}
                    className="w-full text-sm text-lightText/60 file:mr-4 file:py-2.5 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-secondaryAction/20 file:text-secondaryAction hover:file:bg-secondaryAction/30 transition-colors file:border-secondaryAction/20"
                  />
                 </div>
              </div>
            </div>
