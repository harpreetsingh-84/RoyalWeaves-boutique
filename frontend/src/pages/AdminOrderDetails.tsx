    ...
    <div className="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-400">Order ID: <span className="font-mono text-gray-200">{order._id}</span> • {orderDate}</p>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-8 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <div className="bg-adminBg p-4 rounded border border-blue-100 mb-4 space-y-2">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-400 font-bold tracking-widest uppercase">Amount Due</span>
                ...
                <div className="flex flex-col text-sm border-t border-blue-50 pt-2">
                  <span className="text-gray-400 font-bold tracking-widest uppercase mb-1">Transaction ID (UTR)</span>
                  <span className="font-mono text-lg font-bold select-all bg-adminCardAlt p-2 rounded tracking-wider">{order.transactionId || 'NOT PROVIDED'}</span>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center border-2 border-dashed border-blue-200 rounded-xl bg-adminBg p-2">
      ...
      {/* Admin Controls Box */}
      <div className="bg-adminCard border border-adminBorderLight rounded-xl p-6 mb-8 shadow-sm">
        <h2 className="text-lg font-bold text-gray-100 mb-4 flex items-center gap-2">
          📦 Logistics Status
        </h2>
        <div className="flex flex-col sm:flex-row gap-4 items-end">
          <div className="flex-grow w-full">
            <label className="block text-sm font-medium text-gray-200 mb-1 leading-none">Order Status</label>
            <select
              value={selectedStatus}
              onChange={(e) => setSelectedStatus(e.target.value)}
              className="w-full bg-adminBg border border-adminBorderLighter text-gray-100 text-sm rounded-lg focus:ring-accent focus:border-accent block p-2.5 capitalize"
            >
          ...
          <button
            ...
            className={`w-full sm:w-auto px-6 py-2.5 rounded-lg font-bold shadow-sm flex-shrink-0 transition-colors
              ${updatingStatus || selectedStatus === order.orderStatus
                ? 'bg-adminBorder text-gray-400 cursor-not-allowed'
      ...
      <div className="bg-adminBg border rounded-xl p-6 mb-8 shadow-sm">
        <h2 className="text-lg font-semibold mb-6">Customer Timeline View</h2>
        <div className="relative opacity-70">
          <div className="absolute top-1/2 left-0 w-full h-1 bg-adminBorder -translate-y-1/2 z-0 hidden sm:block"></div>
          <div
          ...
          <div className="relative z-10 flex flex-col sm:flex-row justify-between gap-6 sm:gap-0">
            {steps.map((step, index) => {
              ...
              return (
                <div key={step} className="flex sm:flex-col items-center gap-4 sm:gap-2 relative bg-adminBg sm:bg-transparent">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-colors duration-300 ${isCompleted ? 'bg-green-500 border-green-500 text-white' : 'bg-adminBg border-adminBorderLighter text-gray-400'}`}>
                    {isCompleted ? (
                  ...
                  <div className={`text-sm sm:text-center ${isCurrent ? 'font-bold text-gray-100' : isCompleted ? 'font-medium text-gray-300' : 'text-gray-400'}`}>
                    {step}
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-adminCard p-6 rounded-xl border">
            ...
            {order.user ? (
              ...
              <p className="text-gray-400 italic">Guest Checkout (No Account)</p>
            )}
          </div>
          ...
          <div className="bg-adminCard p-6 rounded-xl border">
            ...
            <div className="text-gray-300 text-sm space-y-1">
              ...
              <p className="pt-2 text-gray-400 flex justify-between">
                <span>Phone:</span>
                <span className="text-gray-200 font-medium">{order.shippingDetails?.phone}</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-adminCard p-6 rounded-xl border">
            ...
            <div className="text-gray-300 text-sm space-y-2">
              <p className="flex justify-between items-center text-gray-400 text-xs font-bold uppercase tracking-widest border-b pb-1 mb-2">
              ...
              <p className="flex justify-between items-center">
                ...
                <span className="font-bold text-gray-100">{order.paymentMethod || 'UPI (Manual)'}</span>
              </p>
              <p className="flex flex-col gap-1 border-t border-adminBorderLight pt-2 mt-2">
                <span className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Transaction / UTR ID</span>
                <span className="font-mono bg-adminBg px-2 py-1 border border-adminBorderLight rounded select-all break-all">{order.transactionId || 'NOT PROVIDED'}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          <div className="bg-adminBg border rounded-xl overflow-hidden shadow-sm">
            <div className="py-4 px-6 border-b bg-adminCard flex justify-between items-center">
              <h3 className="text-lg font-semibold">Ordered Products</h3>
              <span className="text-sm font-medium bg-adminBorder px-2 py-0.5 rounded-full">{order.items?.length || 0} Items</span>
            </div>
            <div className="divide-y">
              {order.items?.map((item: any, idx: number) => (
                <div key={idx} className="p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6">
                  <div className="w-20 h-20 bg-adminCardAlt rounded-lg overflow-hidden flex-shrink-0 border">
                    {item.product?.image || item.image ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-gray-400 bg-adminCard">
                        <span className="text-[10px] uppercase font-bold tracking-widest text-gray-300">No Img</span>
                      </div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <Link to={`/item/${item.product?._id}`} className="text-base font-bold text-gray-100 hover:text-blue-600 hover:underline truncate block">
                      {item.name}
                    </Link>
                    <div className="mt-1 text-xs font-mono text-gray-400">
                      ProductID: {item.product?._id || 'Unknown'}
                    </div>
                    <div className="mt-2 text-sm text-gray-300 flex items-center gap-4 bg-adminCard inline-flex px-3 py-1 rounded-full border">
            ...
            <div className="p-6 bg-adminCard border-t">
              <div className="space-y-3 ms-auto max-w-sm w-full">
                ...
                <div className="border-t border-adminBorderLight pt-3 mt-3 flex justify-between font-black text-xl text-gray-100">
